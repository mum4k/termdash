...
func TestAssetSceneUsesProvidedModel(t *testing.T) {
	asset := CreateCube(Vector3D{}, 1, '█')
	asset.SetColor(Color{R: 0.9, G: 0.4, B: 0.2})
...
// TestCenteredGlyphRowsKeepsBoardCentered verifies scene boards aren't clipped
// by stale hand-tuned left origins when their text changes.
func TestCenteredGlyphRowsKeepsBoardCentered(t *testing.T) {
	rows := []string{
		"╭────╮",
		"│ ABC│",
		"╰────╯",
	}
	model := createCenteredGlyphRows(Vector3D{X: 0, Y: 0, Z: 0}, rows, 0.1, 0.2, graphColor)
	if model == nil || len(model.Faces) == 0 {
		t.Fatal("createCenteredGlyphRows() returned empty model")
	}
	minX, maxX := 1.0, -1.0
	for _, face := range model.Faces {
...
func TestNormalizeShowcaseAssetModelCentersGeometry(t *testing.T) {
	model := CreateCube(Vector3D{X: 10, Y: 5, Z: -3}, 8, '█')
...
func TestMergeModelsCountsFaces(t *testing.T) {
	left := CreateCube(Vector3D{}, 1, '█')
	right := CreateCube(Vector3D{X: 2}, 1, '█')
